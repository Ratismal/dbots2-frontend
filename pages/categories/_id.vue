<template>
  <div>
    <section class="section">
      <div class="container">
        <h1 class="title">
          Discover {{ categoryName }} Bots
        </h1>
        <h2 class="subtitle">
          Find the perfect bot for your server
        </h2>
      </div>
      <hr>
      <div class="container">
        <bot-list-item-grid :bots="bots"/>
      </div>
    </section>
  </div>
</template>

<script>
import BotListItemGrid from "~/components/BotListItemGrid.vue"

const VALID_CATEGORIES = ["anime", "fun", "games", "moderation", "music", "nsfw", "utility"];
VALID_CATEGORIES.push("gaming"); // TODO debug only, remove

export default {
  components: {
    BotListItemGrid
  },
  validate(ctx) {
    return VALID_CATEGORIES.includes(ctx.params.id);
  },
  head() {
    return {
      title: `${this.categoryName[0].toUpperCase()}${this.categoryName.substring(1)} Bots | Discord Bots`
    }
  },
  middleware: "betaOnly",
  data() {
    return {
      categoryName: this.$route.path.replace(/^\/categories\//, "")
    };
  },
  asyncData(ctx) {
    return ctx.$axios.$get("/bots", {
      params: {
        category: ctx.params.id
      }
    }).then((bots) => {
      return {
        bots: bots
      };
    }).catch((err) => {
      if(err && err.response && err.response.status) {
        return ctx.error({
          statusCode: err.response.status,
          message: err.response.data && err.response.data.message || err.message || "Internal error"
        });
      }
      return ctx.error({
        statusCode: 500,
        message: err.message || "Internal error"
      });
    });
  }
}
</script>

<style>
</style>
