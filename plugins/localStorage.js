function persistStore(key, paths, store) {
  ...
  store.subscribe((mut, state) => {
    let savePaths = !paths.length ? state : paths.reduce((m, v) => Object.assign(m, {[v]: state[v]}), {}); // Reduces the save state to what paths we want to persist.
...
export default ({ store, isHMR }) => {
  if (isHMR) {
    return; // Stops the persistance thing from being reapplied when HMR happens.
  }
